...
bool compute_selection(PropmatVector&                                   pm [[maybe_unused]],
                       ...
                       const Absorption::PredefinedModel::ModelVariant& predefined_model_data [[maybe_unused]]) try {
  switch (Species::find_species_index(model)) {
    case "H2O-ForeignContCKDMT400"_isot_index:
      if constexpr (not check_exist)
        MT_CKD400::compute_foreign_h2o(pm, f, atm_point, std::get<MT_CKD400::WaterData>(predefined_model_data.data));
      return true;
    case "H2O-SelfContCKDMT400"_isot_index:
      if constexpr (not check_exist)
        MT_CKD400::compute_self_h2o(pm, f, atm_point, std::get<MT_CKD400::WaterData>(predefined_model_data.data));
      return true;
    case "H2O-ForeignContCKDMT430"_isot_index:
      if constexpr (not check_exist)
        MT_CKD430::compute_foreign_h2o(pm, f, atm_point, std::get<MT_CKD430::WaterData>(predefined_model_data.data));
      return true;
    case "H2O-SelfContCKDMT430"_isot_index:
      if constexpr (not check_exist)
        MT_CKD430::compute_self_h2o(pm, f, atm_point, std::get<MT_CKD430::WaterData>(predefined_model_data.data));
      return true;
    ...
    case "H2O-PWR2021"_isot_index:
      if constexpr (not check_exist) PWR20xx::compute_h2o_2021(pm, f, atm_point);
      return true;
    ...
    case "H2O-PWR2022"_isot_index:
      if constexpr (not check_exist) PWR20xx::compute_h2o_2022(pm, f, atm_point);
      return true;
    ...
    case "H2O-ForeignContCKDMT350"_isot_index:
      if constexpr (not check_exist) CKDMT350::compute_foreign_h2o(pm, f, atm_point);
      return true;
    case "H2O-SelfContCKDMT350"_isot_index:
      if constexpr (not check_exist) CKDMT350::compute_self_h2o(pm, f, atm_point);
      return true;
    case "H2O-ForeignContCKDMT320"_isot_index:
      if constexpr (not check_exist) CKDMT320::compute_foreign_h2o(pm, f, atm_point);
      return true;
    case "H2O-SelfContCKDMT320"_isot_index:
      if constexpr (not check_exist) CKDMT320::compute_self_h2o(pm, f, atm_point);
      return true;
    ...
    case "CO2-CKDMT252"_isot_index:
      if constexpr (not check_exist) MT_CKD252::carbon_dioxide(pm, f, atm_point);
      return true;
  ...
  throw std::runtime_error(std::format("Data for model {} is not of correct type", model));
...
void compute(PropmatVector&                                   propmat_clearsky,
  ...
  if (not compute_selection<true>(propmat_clearsky, model, f_grid, atm_point, predefined_model_data)) return;
  ...
  const bool do_freq_jac = stdr::any_of(freq_jac, [e](auto& x) { return x != e; });

  const bool do_temp_jac = temp_jac != e;

  const bool do_vmrs_jac = stdr::any_of(vmrs_jac, [e](auto& x) { return x != e; });

  if (do_freq_jac or do_temp_jac or do_vmrs_jac) {
    PropmatVector pm(f_grid.size());
    PropmatVector dpm(f_grid.size());
    compute_selection<false>(pm, model, f_grid, atm_point, predefined_model_data);
    ...
    for (auto& j : vmrs_jac) {
      if (j != e) {
        ...
        const auto    iq = j->target_pos;
        compute_vmr_deriv(
            dpm, pm, model, f_grid, atm_point, d, *std::get_if<SpeciesEnum>(&j->type), predefined_model_data);
        dpropmat_clearsky_dx[iq] += dpm;
      }
    }
  } else {
    compute_selection<false>(propmat_clearsky, model, f_grid, atm_point, predefined_model_data);
  }
}
}  // namespace Absorption::PredefinedModel