...
constexpr std::pair<std::array<Index, N>, T> to_pair_i(std::array<Index, N>& ret, arg&& x, args&&... y) {
...
template <typename T, Size N, typename... args> constexpr std::pair<std::array<Index, N>, T> to_pair(args&&... y) {
  std::array<Index, N> ret{};
  return to_pair_i<T, N, 0>(ret, std::forward<args>(y)...);
}
}  // namespace

template <typename T, Size N> class [[nodiscard]] data_t {
  ...
  void reset_view(const std::array<Index, N>& sz) noexcept { view.base_set({data.data(), sz}); }

  template <typename U, Size M> friend class data_t;
  template <typename U, Size M> friend struct view_t;
  template <typename U, Size M> friend struct strided_view_t;
 ...
 public:
  ...
  constexpr data_t(data_t&& x) noexcept : data(std::move(x.data)), view(base{data.data(), x.shape()}) { x.clear(); }

  template <integral... inds> explicit constexpr data_t(inds... ind) requires(sizeof...(inds) == N)
      : data_t(std::array{static_cast<Index>(ind)...}) {}

  template <class... args> constexpr data_t(args... arg) requires(sizeof...(args) == (N + 1))
      : data_t(to_pair<T, N>(std::forward<args>(arg)...)) {}

  explicit constexpr data_t(std::vector<T>&& r) requires(N == 1) : data(std::move(r)), view(data) {}

  data_t(std::initializer_list<T> r) requires(N == 1) : data_t(std::vector<T>(stdr::begin(r), stdr::end(r))) {}

  data_t(std::from_range_t, stdr::input_range auto&& r) requires(N == 1) {
    std::vector<T> v;
  ...
  template <mdvalue_type_compatible<T> U> explicit constexpr data_t(const U& x) : data_t(mdshape(x)) { view = x; }

  template <typename U> explicit constexpr data_t(const view_t<U, N>& x) : data_t(x.shape()) { view = x; }

  template <typename U> explicit constexpr data_t(const strided_view_t<U, N>& x) : data_t(x.shape()) { view = x; }

  template <ranked_convertible_md<T, N> Real, ranked_convertible_md<T, N> Imag>
  constexpr data_t(const Real& real, const Imag& imag)
      requires(complex_type<T> and std::same_as<typename Real::value_type, complex_subtype_t<T>> and
               std::same_as<typename Imag::value_type, complex_subtype_t<T>>)
      : data_t(real.shape()) {
    assert(real.shape() == imag.shape());
    auto r = real | by_elem;
    auto i = imag | by_elem;
    std::transform(stdr::begin(r), stdr::end(r), stdr::begin(i), data.begin(), [](auto a, auto b) { return T{a, b}; });
  }
  ...
  template <integral... inds> constexpr data_t& resize(inds... ind) requires(sizeof...(inds) == N) {
  ...
  template <ranked_convertible_md<T, N> R> constexpr data_t& operator=(const R& r)
  ...
  template <mdvalue_type_compatible<T> U> constexpr data_t& operator=(const U& x)
      requires(not(std::same_as<data_t, U> or std::same_as<value_type, U> or ranked_convertible_md<U, T, N>)) {
    *this = data_t(x);
    return *this;
  }

  template <Size M> constexpr data_t<T, M> reshape(const std::array<Index, M>& exts) && {
    data_t<T, M> out{};
  ...
  template <integral... inds> constexpr auto reshape(inds... exts) && {
    return std::move(*this).template reshape<sizeof...(inds)>(std::array{static_cast<Index>(exts)...});
  }
  ...
  template <std::convertible_to<T> U> constexpr void push_back(U&& x) requires(N == 1) {
    data.push_back(static_cast<T>(std::forward<U>(x)));
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  template <stdr::forward_range U> constexpr void append_range(U&& x)
      requires(N == 1 and std::convertible_to<stdr::range_value_t<U>, T>) {
    data.append_range(std::forward<U>(x));
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  constexpr void pop_back() requires(N == 1) {
    assert(not data.empty());
    data.pop_back();
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  constexpr void erase(std::vector<T>::iterator pos) requires(N == 1) {
    assert(not data.empty());
    data.erase(pos);
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  constexpr void erase(std::vector<T>::const_iterator pos) requires(N == 1) {
    assert(not data.empty());
    data.erase(pos);
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  constexpr void erase(std::vector<T>::iterator first, std::vector<T>::iterator last) requires(N == 1) {
    assert(not data.empty());
    data.erase(first, last);
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }

  constexpr void erase(std::vector<T>::const_iterator first, std::vector<T>::const_iterator last) requires(N == 1) {
    assert(not data.empty());
    data.erase(first, last);
    reset_view(std::array<Index, N>{static_cast<Index>(data.size())});
  }
  ...
  template <class... Args> constexpr T& emplace_back(Args&&... args) requires(N == 1) {
  ...
  template <typename U> constexpr data_t& operator+=(U&& x) {
  ...
  template <typename Self> constexpr auto shape(this Self&& self) { return std::forward<Self>(self).view.shape(); }
  ...
  template <typename Self> constexpr auto extents(this Self&& self) { return std::forward<Self>(self).view.extents(); }

  template <typename Self> constexpr auto mapping(this Self&& self) { return std::forward<Self>(self).view.mapping(); }
...
template <typename T, Size N> struct std::formatter<matpack::data_t<T, N>> {
  ...
  template <class FmtContext> FmtContext::iterator format(const matpack::data_t<T, N>& v, FmtContext& ctx) const {
