...
is considered a documentation file and will be processed.
...
2.  The sorting of all the content is alphanumerical.  Put ``1*``, ``2*``,
    etc., in front of the filenames to control the order if you think it
    matters.
...
generated documentation for the workspace variables, methods, agendas, and
...
2. Several C++-files contain raw text that will be included in the
   ...
   d. ``src/workspace_meta_methods.cc`` - contain workspace methods'
      documentation that is composed by calling other methods.
   ...
   f. ``src/workspace_agendas.cc`` - contains workspace agendas'
      documentation. This includes constraints that are both documentation
   ...
   g. ``src/core/options/arts_options.cc`` - contains the documentation
      ...
      of ARTS.  These are workspace groups, but they will generate
...
3. Only workspace groups document any information beyond
   ...
   a. The additional information is a table view of all
      ...
      expand it for the chosen double-underscore methods.
...
5. Workspace methods' builtin documentation states whether or not
