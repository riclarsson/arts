...
DISORT solves the scalar plane-parallel radiative-transfer equation.
VDISORT solves its polarized, Stokes-vector form.
...
Validation of the scalar limit is described in :doc:`dev.disort`.
...
Implementation storage layouts are described in :doc:`dev.disort`.
...
so this DFDT is with respect to physical optical depth.

See :doc:`dev.disort` for implementation limits and validation coverage.
...
See :doc:`user.disort` for input and output conventions.
