...
C   ANY DAMAGES THAT MAY RESULT FROM THE USE OF THE PROGRAM. 
      ...
      SUBROUTINE Tmatrix(RAT,AXI,NP,DLAM,DEPS,DMRR,DMRI,DDELT,QUIET,
...
C     Library calls must not redirect the process stdout.
...
C      DDELT=DDDELT
      ...
      if (quiet /= 1) PRINT 5454, NCHECK
 5454 FORMAT ('NCHECK=',I1)
      IF (DABS(RAT-1D0).GT.1D-8.AND.NP.EQ.-1) CALL ASAREA (DEPS,RAT)
      IF (DABS(RAT-1D0).GT.1D-8.AND.NP.GE.0) CALL ASURFCH(NP,DEPS,RAT)
      IF (DABS(RAT-1D0).GT.1D-8.AND.NP.EQ.-2) CALL ASAREAC (DEPS,RAT)
...
!for randomly oriented particles
      CSCA=QSCA*DLAM**2/2/P
      CEXT=-QEXT*DLAM**2/2/P
...
C**********************************************************************
   ...
   10 CONTINUE
      NG=2*NGAUSS
      IF (NP.EQ.-2) GO  TO 11
      CALL AQGAUSS(NG,0,0,X,W)
      GO TO 19
   11 NG1=DFLOAT(NGAUSS)/2D0
      NG2=NGAUSS-NG1
      XX=-QCOS(QATAN(EPS))
      CALL AQGAUSS(NG1,0,0,X1,W1)
      CALL AQGAUSS(NG2,0,0,X2,W2)
      DO 12 I=1,NG1
...
C***************************************************************
      ...
      SUBROUTINE AQGAUSS ( N,IND1,IND2,Z,W )
...
C**********************************************************************
     ...
     *        DY(NPNG2,NPN1)
      ...
      IF (NP.GE.0) CALL ARSP2(X,NG,A,EPS,NP,R,DR)
